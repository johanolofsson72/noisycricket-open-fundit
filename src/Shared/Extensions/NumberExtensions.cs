...
public static class NumberExtensions
{
    ...
    public static string FormatNumber(this decimal numberToFormat, int decimalPlaces = 2)
    {
        ...
        foreach (NumberSuffix suffix in Enum.GetValues<NumberSuffix>())
        {
            ...
            if (numberToFormat >= (decimal)currentValue)
            {
                numberString = $"{Math.Round(numberToFormat / (decimal)currentValue, decimalPlaces, MidpointRounding.ToEven)} {suffixValue}";
            }
        }
