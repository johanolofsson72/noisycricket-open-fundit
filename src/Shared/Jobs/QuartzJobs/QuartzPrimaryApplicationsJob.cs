...
public class QuartzPrimaryApplicationsJob(IServiceScopeFactory serviceScopeFactory) : IJob
{
    public static readonly JobKey Key = new JobKey("QuartzPrimaryApplications", "SingleJob");
    ...
    public async Task Execute(IJobExecutionContext jobExecutionContext)
    {
        try
        {
            ...
            var statusImportant = new List<int> { 3, 4, 5, 7, 8, 9, 10, 11, 12, 13, 14, 15, 22, 23, 24 };
            ...
            var applications = await context.Applications
                .AsTracking()
                .Where(x => statusImportant.Contains(x.StatusId))
            ...
            Console.WriteLine($"<= QuartzPrimaryApplications" + $", total processing time: {timer.Elapsed:mm\\:ss}");
            
        }
        catch (Exception ex)
        {
            Console.WriteLine("<= QuartzPrimaryApplications failed " + ex);
        }
    }
