...
public class QuartzDocumentsJob(IServiceScopeFactory serviceScopeFactory) : IJob
{
    public static readonly JobKey Key = new JobKey("QuartzDocuments", "SingleJob");
    ...
    public async Task Execute(IJobExecutionContext jobExecutionContext)
    {
        try
        {
            ...
            Console.WriteLine($"<= QuartzDocuments started" + $", at: {DateTime.UtcNow:hh:mm:ss}");
        
            // Get documents
            var documents = await context.Documents
                .AsNoTracking()
                .Where(x => x.Phrases.Length < 5 && x.Summarize.Length < 5)
                .ToListAsync();
            ...
            {
                if (document.Path.Length <= 0 && document.Binary.Length <= 0) continue;
            ...
            // Clear Cache
            if (changed) await cache.RemoveByPrefixAsync(CacheKeyPrefix.Documents.ToDescriptionString());
            
            Console.WriteLine($"<= QuartzDocuments succeeded" + $", total processing time: {timer.Elapsed:mm\\:ss}");
            
        }
        catch (Exception ex)
        {
            Console.WriteLine("<= QuartzDocuments failed " + ex);
        }
    }
