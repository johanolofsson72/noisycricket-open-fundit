...
public class QuartzApplicationJob(IServiceScopeFactory serviceScopeFactory) : IJob
{
    public static readonly JobKey Key = new JobKey("QuartzApplication", "SingleJob");
    ...
    public async Task Execute(IJobExecutionContext jobExecutionContext)
    {
        try
        {
            ...
            Console.WriteLine($"<= QuartzApplication started for id: {applicationId}" + $", at: {DateTime.UtcNow:hh:mm:ss}");
        
            // Get Application
            var application = await context.Applications
                .AsTracking()
                .FirstOrDefaultAsync(x => x.Id == applicationId) ?? throw new Exception($"Application: {applicationId} not found");
            
            Console.WriteLine($"<= QuartzApplication has project id: {application.ProjectId}" + $", at: {DateTime.UtcNow:hh:mm:ss}");
            
            // Construct Values
            var schemaNames = GetSchemaName(context, application.SchemaId);
            var projectNumber = GetProjectNumber(context, application.ProjectId);
            var organization = GetOrganization(context, application.Organization.ContactIdentifier);
            var producer = new UserSummary();
            if (application.Producer.Name.Length > 0)
            {
                application.Producer.ContactIdentifier = await CheckPerson(context, organization, application.Producer);
            ...
            {
                application.Applicant.ContactIdentifier = await CheckPerson(context, organization, application.Applicant);
            ...
            {
                application.ProjectManager.ContactIdentifier = await CheckPerson(context, organization, application.ProjectManager);
            ...
            {
                application.ProductionManager.ContactIdentifier = await CheckPerson(context, organization, application.ProductionManager);
            ...
            {
                application.FinanceManager.ContactIdentifier = await CheckPerson(context, organization, application.FinanceManager);
            ...
            {
                application.ScriptManager.ContactIdentifier = await CheckPerson(context, organization, application.ScriptManager);
            ...
            {
                application.DistributionManager.ContactIdentifier = await CheckPerson(context, organization, application.DistributionManager);
            ...
            {
                application.ContractManager.ContactIdentifier = await CheckPerson(context, organization, application.ContractManager);
            ...
            application.ProjectNumber = projectNumber;
            application.SchemaNames = schemaNames.ToList();
            ...
            applicationTitle = applicationTitle is not null ? Regex.Unescape(applicationTitle) : applicationTitle;
            ...
            applicationBudget = applicationBudget is not null ? Regex.Unescape(applicationBudget) : applicationBudget;
            _ = decimal.TryParse(applicationBudget, out var applicationBudgetD);
            if (applicationBudget != null) application.BudgetAmount = applicationBudgetD;
            
            var applicationApplied = application.Controls.FirstOrDefault(x => x.UniqueId.ToString().StartsWith("00000002"))?.Value;
            applicationApplied = applicationApplied is not null ? Regex.Unescape(applicationApplied) : applicationApplied;
            _ = decimal.TryParse(applicationApplied, out var applicationAppliedD);
            if (applicationApplied != null) application.AppliedAmount = applicationAppliedD;
            
            var applicationOur = application.Controls.FirstOrDefault(x => x.UniqueId.ToString().StartsWith("01000001"))?.Value;
            applicationOur = applicationOur is not null ? Regex.Unescape(applicationOur) : applicationOur;
            _ = decimal.TryParse(applicationOur, out var applicationOurD);
            if (applicationOur != null) application.OurContribution = applicationOurD;
            ...
            if (application.ProjectId > 0)
            {
                ...
                var applications = GetProjectApplications(context, application.ProjectId).ToList();
                
                // Get Project
                var project = await context.Projects
                    .AsTracking()
                    .FirstOrDefaultAsync(x => x.Id == application.ProjectId) ?? throw new Exception($"Project: {application.ProjectId} not found for application: {applicationId}");
                
                // Set Project Values
                project.Applications = applications;
                project.ApplicationCount = applications.Count;
                ...
                applicationEarlierTitle = applicationEarlierTitle is not null ? Regex.Unescape(applicationEarlierTitle) : applicationEarlierTitle;
            ...
            Console.WriteLine($"<= QuartzApplication succeeded for id: {applicationId}" + $", total processing time: {timer.Elapsed:mm\\:ss}");
            
            Console.WriteLine($"[QuartzApplication] ProjectId: {application.ProjectId}");
            
            if (application.ProjectId == 0) return;
            
            var schedulerFactory = scope.ServiceProvider.GetRequiredService<ISchedulerFactory>();
            var scheduler = await schedulerFactory.GetScheduler(jobExecutionContext.CancellationToken);
            
            // Trigger OpenAiProject
            var openAiProjectJob = JobBuilder.Create<QuartzOpenAiProjectJob>()
                .WithIdentity($"QuartzOpenAiProjectJobFromQuartzApplication{application.ProjectId}", "SingleJob")
            ...
            var openAiProjectTrigger = TriggerBuilder.Create()
                .WithIdentity($"QuartzOpenAiProjectTriggerFromQuartzApplication{application.ProjectId}", "SingleTrigger")
                .StartNow()
                .Build();

            await scheduler.ScheduleJob(openAiProjectJob, openAiProjectTrigger, jobExecutionContext.CancellationToken);
            Console.WriteLine($"[QuartzApplication] Scheduled QuartzOpenAiProjectJob with ProjectId: {application.ProjectId}");

            // Trigger Notification
            if (notificationType == NotificationType.Default) return;
                
            var notificationJob = JobBuilder.Create<QuartzNotificationJob>()
                .WithIdentity($"QuartzNotificationJobFromQuartzApplication{applicationId}", "SingleJob")
                .SetJobData(new JobDataMap()
                {
                    ["projectId"] = application.ProjectId,
            ...
            var notificationTrigger = TriggerBuilder.Create()
                .WithIdentity($"QuartzNotificationTriggerFromQuartzApplication{applicationId}", "SingleTrigger")
                .StartNow()
                .Build();

            await scheduler.ScheduleJob(notificationJob, notificationTrigger, jobExecutionContext.CancellationToken);
            Console.WriteLine($"[QuartzApplication] Scheduled QuartzNotificationJob with ProjectId: {application.ProjectId}, ApplicationId: {applicationId}, NotificationType: {notificationType}");

        }
        catch (Exception ex)
        {
            Console.WriteLine("<= QuartzApplication failed " + ex);
        }
    }
    ...
    private async Task<int> CheckPerson(ApplicationDbContext context, OrganizationSummary organization, ApplicationContact person)
    {
        if (person.ContactIdentifier != 0) return person.ContactIdentifier;
        
        var oldUser = await context.Users.AsNoTracking().FirstOrDefaultAsync(x => x.Email == person.Email.ToLower());
        if (oldUser != null) return oldUser.Id;
        ...
        var user = new User
        {
            ...
            PhoneNumbers = [ new UserPhoneNumber { PhoneNumberIdentifier = 1, Number = person.PhoneNumber, Type = 1 } ],
            Organizations =
            [
