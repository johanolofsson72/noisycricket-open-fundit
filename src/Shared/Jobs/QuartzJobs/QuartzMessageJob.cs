...
public class QuartzMessageJob(IServiceScopeFactory serviceScopeFactory) : IJob
{
    ...
    public async Task Execute(IJobExecutionContext jobExecutionContext)
    {
        try
        {
            ...
            Console.WriteLine($"<= QuartzMessage started for id: {messageId}" + $", at: {DateTime.UtcNow:hh:mm:ss}");

            // Get message
            var message = await context.Messages
                .AsTracking()
                .FirstOrDefaultAsync(x => new List<int>(){16, 19, 23}.Contains(x.StatusId) == false) ?? throw new Exception("Can't find messages");
            ...
            Console.WriteLine($"<= QuartzMessage succeeded for id: {messageId}" + $", total processing time: {timer.Elapsed:mm\\:ss}");
            
        }
        catch (Exception ex)
        {
            Console.WriteLine("<= QuartzMessage failed " + ex);
        }
    }
