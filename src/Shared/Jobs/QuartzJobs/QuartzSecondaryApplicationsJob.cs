...
public class QuartzSecondaryApplicationsJob(IServiceScopeFactory serviceScopeFactory) : IJob
{
    public static readonly JobKey Key = new JobKey("QuartzSecondaryApplications", "SingleJob");
    ...
    public async Task Execute(IJobExecutionContext jobExecutionContext)
    {
        try
        {
            ...
            var statusNormal = new List<int> { 16, 17, 18, 21 };
            ...
            var applications = await context.Applications
                .AsTracking()
                .Where(x => statusNormal.Contains(x.StatusId))
            ...
            Console.WriteLine($"<= QuartzSecondaryApplications" + $", total processing time: {timer.Elapsed:mm\\:ss}");
            
        }
        catch (Exception ex)
        {
            Console.WriteLine("<= QuartzSecondaryApplications failed " + ex);
        }
    }
